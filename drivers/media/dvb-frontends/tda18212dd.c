...
static int StartCalibration(struct tda_state *state)
{
	...
	do {
		...
		status = update_reg(state, POWER_2);
		if (status < 0)
			break;

		/* AGC1 Do Step = 2 */
		state->Regs[AGC1_2] = (state->Regs[AGC1_2] & ~0x60) | 0x40;
		status = update_reg(state, AGC1_2);        /* AGC */
		if (status < 0)
			break;
...
static int FinishCalibration(struct tda_state *state)
{
	...
	do {
		...
		while (true) {
			status = read_reg(state, IRQ_STATUS, &IRQ);
			if (status < 0)
				break;
			if ((IRQ & 0x80) != 0)
				break;
		...
		status = update_reg(state, FLO_MAX);
		if (status < 0)
			break;
		...
		state->Regs[AGC1_1] |= (state->m_isMaster ?
					MASTER_AGC1_6_15dB :
					SLAVE_AGC1_6_15dB) << 6;
		status = update_reg(state, AGC1_1);
		if (status < 0)
			break;
		...
		status = update_reg(state, PSM_1);
		if (status < 0)
			break;

		state->Regs[REFERENCE] |= 0x03; /* XTOUT = 3 */
		status = update_reg(state, REFERENCE);
		if (status < 0)
			break;

		status = read_regs(state, RFCAL_LOG_1, RFCal_Log,
			sizeof(RFCal_Log));
		if (status < 0)
			break;
	} while (0);
...
static int Standby(struct tda_state *state)
{
	...
	do {
		...
		status = update_reg(state, POWER_STATE_2);
		if (status < 0)
			break;
	} while (0);
	return status;
...
static int PowerMeasurement(struct tda_state *state, u8 *pPowerLevel)
{
	...
	do {
		...
		status = read_reg1(state, INPUT_POWER_LEVEL);
		if (status < 0)
			break;
			...
			enum HF_Standard Standard)
{
	...
	do {
		...
		status = update_reg(state, AGC2_1);
		if (status < 0)
			break;
		...
		status = update_reg(state, RF_AGC_1);
		if (status < 0)
			break;
		...
		status = update_reg(state, IR_MIXER_1);
		if (status < 0)
			break;
		...
		state->Regs[IF_FREQUENCY_1] = (StandardParams->m_IFFrequency /
					       50000);
		status = update_reg(state, IF_FREQUENCY_1);
		if (status < 0)
			break;
		...
		if (state->m_isMaster && StandardParams->m_LTO_STO_immune) {
			...
			status = read_reg(state, RF_AGC_GAIN_1, &tmp);
			if (status < 0)
				break;
			...
			state->Regs[RF_FILTER_1] =
				...
				(RF_Filter_Gain << 2);
			status = update_reg(state, RF_FILTER_1);
			if (status < 0)
				break;
		...
		status = update_regs(state, RF_FREQUENCY_1, RF_FREQUENCY_3);
		if (status < 0)
			break;
		...
		if (StandardParams->m_AGC1_Freeze && state->m_bEnableFreeze) {
			...
			status = read_reg(state, AGC1_2, &tmp);
			if (status < 0)
				break;
			if ((tmp & 0x80) == 0) {
				state->Regs[AGC1_2] |= 0x80;    /* Loop off */
				status = update_reg(state, AGC1_2);
				if (status < 0)
					break;
			...
			while (Step < nSteps) {
				...
				for (i = 0; i < 40; i += 1) {
					status = read_reg(state, AGC_DET_OUT,
						&tmp);
					if (status < 0)
						break;
					Up   += (tmp & 0x02) ?  1 : -4;
